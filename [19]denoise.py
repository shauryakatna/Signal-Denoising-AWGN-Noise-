...
l2_reg = regularizers.l2(0.001)  # Adjusted regularization term
...
x = layers.Conv1D(512, kernel_size=3, activation='relu', padding='same', kernel_regularizer=l2_reg)(x)
x = layers.BatchNormalization()(x)
x = layers.MaxPooling1D(pool_size=2, padding='same')(x)
x = layers.Dropout(0.3)(x)
...
x = layers.Dense((window // 8) * 512, activation='relu', kernel_regularizer=l2_reg)(decoder_input)
x = layers.Reshape((window // 8, 512))(x)
x = layers.UpSampling1D(size=2)(x)
x = layers.Conv1DTranspose(512, kernel_size=3, activation='relu', padding='same', kernel_regularizer=l2_reg)(x)
x = layers.BatchNormalization()(x)
x = layers.Dropout(0.3)(x)
