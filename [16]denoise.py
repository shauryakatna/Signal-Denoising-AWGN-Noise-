...
x = layers.Dense((window // 8) * 256, activation='relu')(decoder_input)
x = layers.Reshape((window // 8, 256))(x)
x = layers.UpSampling1D(size=2)(x)
x = layers.Conv1DTranspose(256, kernel_size=3, activation='relu', padding='same')(x)
x = layers.BatchNormalization()(x)
x = layers.Dropout(0.3)(x)
...
vae = VAE(encoder=keras.Model(inputs, [z_mean, z_log_var]), decoder=decoder)
...
callbacks = [
    ...
    keras.callbacks.ModelCheckpoint(filepath='vae_denoising_best_model.keras', save_best_only=True)
]
